...
import { Users, TrendingUp, Calendar, PhoneCall, AlertCircle, IndianRupee, Activity, CheckCircle } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const paymentPending = pipelineData.filter(p => p.lifecycle_stage === 'Payment Pending').length;
  const paid = pipelineData.filter(p => p.lifecycle_stage === 'Paid').length;
  
  // Dynamic Funnel Data from Live Database
  const funnelData = [
    { label: 'Total Leads', value: totalLeads, color: '#3b82f6' },
    { label: 'New / Uncontacted', value: newLeads, color: '#f59e0b' },
    { label: 'Follow-ups Active', value: followUp, color: '#ec4899' },
    { label: 'Payment Pending', value: paymentPending, color: '#8b5cf6' },
    { label: 'Closed / Paid', value: paid, color: '#10b981' }
  ];
  
  const maxFunnel = Math.max(...funnelData.map(d => d.value), 1); // Avoid division by zero

  return (
    <AdminLayout title="Founder Dashboard">
      <div className="adm-bento-grid">
        
        {/* BIG KPI METRIC: Total Leads */}
        <a href="/admin/pipeline" className="adm-card-glass adm-bento-item" style={{ gridColumn: 'span 4' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--adm-blue)' }}>
            <Users size={24} />
            <span className="adm-metric-label">Total Leads Database</span>
          </div>
          ...
          <p style={{ color: 'var(--adm-text-secondary)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Activity size={16} color="var(--adm-blue)" /> Live tracking from all sources
          </p>
        </a>

        {/* ACTIVE SALES */}
        <a href="/admin/action-center" className="adm-card-glass adm-bento-item" style={{ gridColumn: 'span 4' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#f59e0b' }}>
            <PhoneCall size={24} />
            <span className="adm-metric-label">Active Sales Pipeline</span>
          </div>
          <div className="adm-metric-value">{newLeads + followUp}</div>
          <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
            <span style={{ fontSize: '0.85rem', background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', padding: '4px 8px', borderRadius: '12px', fontWeight: 'bold' }}>{newLeads} New</span>
            <span style={{ fontSize: '0.85rem', background: 'rgba(236, 72, 153, 0.1)', color: '#ec4899', padding: '4px 8px', borderRadius: '12px', fontWeight: 'bold' }}>{followUp} Follow-ups</span>
          </div>
        </a>
        ...
        <div className="adm-card-glass adm-bento-item" style={{ gridColumn: 'span 4' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--adm-green)' }}>
            <IndianRupee size={24} />
            <span className="adm-metric-label">Closed Won Revenue</span>
          </div>
          <div className="adm-metric-value" style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'flex', alignItems: 'center' }}>
            ₹{totalRevenue.toLocaleString()}
          </div>
          <p style={{ color: 'var(--adm-text-secondary)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
            <CheckCircle size={14} color="var(--adm-green)" /> From {paid} enrolled students
          </p>
        </div>
        ...
        <div className="adm-card-glass adm-bento-item" style={{ gridColumn: 'span 8', minHeight: '340px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <TrendingUp size={24} color="var(--adm-text)" />
              <h2 style={{ margin: 0, fontSize: '1.25rem' }}>Global Sales Funnel</h2>
            </div>
          </div>
        ...
        <div className="adm-card-glass adm-bento-item" style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <h2 style={{ margin: 0, fontSize: '1.25rem', display: 'flex', alignItems: 'center', gap: '8px' }}><Calendar size={20} /> Operational Capacity</h2>
          ...
          <div style={{ background: 'rgba(0,0,0,0.02)', padding: '16px', borderRadius: '12px', border: '1px solid var(--adm-border)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontWeight: 600 }}>Global Sales Queue</span>
        ...
        <div className="adm-card-glass adm-bento-item" style={{ gridColumn: 'span 12' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            <h2 style={{ margin: 0, fontSize: '1.25rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <AlertCircle size={20} /> Latest Leads Added
            </h2>
            <button onClick={() => navigate('/admin/pipeline')} style={{ color: 'var(--adm-blue)', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>View Global Pipeline &rarr;</button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
            {pipelineData.slice(0, 8).map((lead, idx) => (
              ...
              >
                <div style={{ fontWeight: 600, fontSize: '1.05rem', marginBottom: '4px' }}>{lead.name}</div>
                <div style={{ fontSize: '0.85rem', color: 'var(--adm-text-secondary)', display: 'flex', justifyContent: 'space-between' }}>
                  <span>{lead.source || 'Direct'}</span>
                  <span style={{ fontWeight: 600, color: lead.lifecycle_stage === 'Paid' ? 'var(--adm-green)' : 'var(--adm-text)' }}>{lead.lifecycle_stage}</span>
                </div>
              </div>
            ))}
            {pipelineData.length === 0 && (
              <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px', color: 'var(--adm-text-secondary)' }}>
                No leads in the database yet. New leads from Meta Ads will appear here instantly.
              </div>
            )}
          </div>
        </div>
