...
export default function MathsRegistration({ onRegister }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const msg = `Hi! My name is ${newRegistration.name}. I'd like to book a free trial session for the Maths Foundation Program.`;
        window.location.href = `https://wa.me/${waNum}?text=${encodeURIComponent(msg)}`;
    };

    return (
        <section id="maths-register" className="register scroll-reveal">
            <div className="sw">
                <div className="register-grid">
                    <div className="register-text scroll-reveal-child">
                        <span className="stag stag-inv">Start Building Confidence</span>
                        <h2 className="sec-h">Book Your Free Trial Session</h2>
                        <p className="sec-p">Experience the E-Brave framework firsthand. We keep our batches extremely small so every student gets focused, <strong>1-on-1 attention</strong>.</p>

                        <div className="reg-benefits">
                            <div className="reg-benefit">
                                <div className="rb-icon">📐</div>
                                <div><strong>Identify Gaps</strong><br /><span>Pinpoint exactly where your child is struggling in Maths</span></div>
                            </div>
                            <div className="reg-benefit">
                                <div className="rb-icon">👩‍🏫</div>
                                <div><strong>1-on-1 Guidance</strong><br /><span>Experience a live, interactive problem-solving session</span></div>
                            </div>
                            <div className="reg-benefit">
                                <div className="rb-icon">🚀</div>
                                <div><strong>Build Confidence</strong><br /><span>See immediate improvement in attitude towards numbers</span></div>
                            </div>
                        </div>
                    ...
                    <div className="register-form-box scroll-reveal-child delay-2">
                        ...
                        <form id="registrationForm" className="styled-form" onSubmit={handleSubmit} noValidate>
                                ...
                                <div className={`input-group ${touched.education && errors.education ? 'has-error' : ''} ${touched.education && !errors.education && formData.education ? 'is-valid' : ''}`}>
                                    ...
                                    <div className="custom-select-wrapper">
                                        <select id="regEducation" required value={formData.education}
                                            ...
                                            {['Class 6','Class 7','Class 8','Class 9','Class 10'].map(o => <option key={o} value={o}>{o}</option>)}
                                        </select>
                                    </div>
                            ...
                            <div className="input-group">
                                <label htmlFor="regProblems">Specific Maths Struggles <span className="optional-tag">(Optional)</span></label>
                                <textarea id="regProblems" rows="3"
                                    placeholder="e.g. My child struggles with algebra and word problems..."
                                    value={formData.problems} onChange={handleChange} maxLength={1000} />
                            </div>

                            <button type="submit" className="btn-main submit-btn" disabled={submitting} style={{ background: 'var(--brand)', color: 'var(--card)' }}>
                                <span>{submitting ? 'Booking your slot…' : 'Book Free Trial'}</span>
                                {!submitting && (
